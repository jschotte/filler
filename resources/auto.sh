...
for map in {0..1};	do
	...
	for j in "abanlin.filler" "carli.filler" "champely.filler" "grati.filler" "hcao.filler" "superjeannot.filler"; do
		...
		for i in {1..5}; do
			...
			./filler_vm -f maps/map0$map -p1 $you -p2 ./players/$j -q | grep fin | GREP_COLOR='01;31' egrep --color=always 'O| ' | GREP_COLOR='01;32' egrep -i --color=always 'X| ';
		done;
		...
		for i in {1..5}; do
			...
			./filler_vm -f maps/map0$map -p2 $you -p1 ./players/$j -q | grep fin | GREP_COLOR='01;31' egrep --color=always 'O| ' | GREP_COLOR='01;32' egrep -i --color=always 'X| ';
		done;
...
if [ $bonus = "y" ]; then
	...
	./filler_vm -f maps/map01 -p1 $you -p2 $you -q | grep fin | GREP_COLOR='01;31' egrep --color=always 'O| ' | GREP_COLOR='01;32' egrep -i --color=always 'X| ';
fi;
echo "Done."
